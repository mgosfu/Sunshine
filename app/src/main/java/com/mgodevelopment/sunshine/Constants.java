package com.mgodevelopment.sunshine;

/**
 * Created by Martin on 10/2/2016.
 */

public class Constants {

    public static final String WEATHER_URL = "http://api.openweathermap.org/data/2.5/forecast/daily?q=%1$s&mode=json&units=%2$s&cnt=%3$d";
    public static final String WEATHER_URL_BASE = "http://api.openweathermap.org/data/2.5/forecast/daily?";
    public static final String OPENWEATHER_API_KEY = "7258b5d9910829802329dd3609900fff";

}
